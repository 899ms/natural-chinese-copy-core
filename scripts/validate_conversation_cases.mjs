...
if (!inputPath) {
  console.error('Missing --file <conversation_move_cases.jsonl>');
  process.exit(2);
}

const __dirname = path.dirname(fileURLToPath(import.meta.url));
const copyValidator = path.join(__dirname, 'validate_chinese_copy.mjs');
const required = ['id', 'move', 'register', 'context', 'bad', 'pass', 'focus'];
...
for (const [index, line] of lines.entries()) {
  ...
  if (!allowedMoves.has(row.move)) {
    issues.push({ severity: 'hard', line: index + 1, id: row.id, reason: 'invalid move' });
  }
  ...
  const tempPath = path.join(process.env.TEMP || process.cwd(), `conversation-pass-${process.pid}-${index}.txt`);
